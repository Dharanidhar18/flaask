...
def classify_audio_clip(audio_clip):
    classifier = AudioMiniEncoderWithClassifierHead(2, spec_dim=1, embedding_dim=512, depth=5, downsample_factor=4,
                                                    resnet_blocks=2, attn_blocks=4, num_attn_heads=4, base_channels=32,
                                                    dropout=0, kernel_size=5, distribute_zero_label=False)
    state_dict = torch.load('classifier.pth', map_location=torch.device('cpu'))
    classifier.load_state_dict(state_dict)
    classifier.eval()
    audio_clip = audio_clip.cpu().unsqueeze(0)
    with torch.no_grad():
        results = classifier(audio_clip)
    return results
...
def classify_audio_clip():
    if request.method == 'POST':
        ...
        print(
            f"Input audio clip shape: {audio_clip.shape}, dtype: {audio_clip.dtype}")
        result = classify_audio_clip(audio_clip)
        return jsonify(result.item())
    if request.method == 'GET':
        file = request.args.get('audio_file')
        audio_clip = load_audio(file)
        result = classify_audio_clip(audio_clip)
        return jsonify([result.item()])
